...
sub update {
    ...
    my @update_keys = keys   %update;
    my @update_vals = values %update;

    foreach my $column (@update_keys) {
    ...
    my ($clause, @clause_vals) = $rows->clause;
    $query .= " $clause";

    my $sth = $dbh->prepare($query);
    edb_bind($sth, @update_vals, @clause_vals);
    $sth->execute;
}
